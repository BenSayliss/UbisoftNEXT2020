...
{	
	...
	void DrawLine(Vec2 sVec, Vec2 eVec, float r, float g, float b)
	{
#if APP_USE_VIRTUAL_RES		
		APP_VIRTUAL_TO_NATIVE_COORDS(sVec.x, sVec.y);
		APP_VIRTUAL_TO_NATIVE_COORDS(eVec.x, eVec.y);
#endif
		glBegin(GL_LINES);
		glColor3f(r, g, b); // Yellow
		glVertex2f(sVec.x, sVec.y);
		glVertex2f(eVec.x, eVec.y);
		glEnd();
	}
